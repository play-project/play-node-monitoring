...
var stats = require('../stats')
  ...
  , metric_collection = measured.createCollection();
...
V1.prototype.start = function start(callback) {
  ...
  setInterval(function() {
    self.emit('fps', metric_collection.toJSON());
  }, 1000);
...
function monitoringStats(req, res, next) {
  stats.new_call(req);
  metric_collection.meter('businessRequestsPerSecond').mark();
  next();
}

function apiStats(req, res, next) {
  stats.new_api_call(req);
  metric_collection.meter('apiRequestsPerSecond').mark();
  next();
}
